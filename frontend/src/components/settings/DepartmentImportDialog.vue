    ...
    <div class="import-content">
      ...
      <div v-if="previewData.length > 0" class="preview-section">
        ...
        <el-table :data="previewData" border size="small" max-height="300">
          ...
          <el-table-column prop="idDep" label="HIS ID" width="200" />
        </el-table>
      </div>
...
import { importDepartments, type ImportResult } from '@/api/department'
...
const previewData = ref<{ code: string; name: string; idDep: string }[]>([])
...
const parsePreview = async (file: File) => {
  try {
    ...
    const idDepIdx = headers.findIndex(h => h === 'id_dep')

    if (codeIdx === -1) {
      ElMessage.error('CSV 文件缺少必需列：CODE')
      return
    }
    if (nameIdx === -1) {
      ElMessage.error('CSV 文件缺少必需列：NAME')
      return
    }
    ...
    for (let i = 1; i < Math.min(lines.length, 11); i++) {
      ...
      previewData.value.push({
        code: values[codeIdx] || '',
        name: values[nameIdx] || '',
        idDep: idDepIdx >= 0 ? values[idDepIdx] || '' : ''
      })
    }
  } catch (error) {
...
const handleImport = async () => {
  ...
  try {
    const result = await importDepartments(selectedFile.value)
