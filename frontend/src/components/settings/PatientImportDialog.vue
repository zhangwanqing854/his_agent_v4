    ...
    <div class="import-content">
      ...
      <div v-if="previewData.length > 0" class="preview-section">
        ...
        <el-table :data="previewData" border size="small" max-height="300">
          ...
          <el-table-column prop="sdSex" label="SD_SEX" width="80" />
          <el-table-column prop="nameSex" label="NAME_SEX" />
          <el-table-column prop="dtBirth" label="DT_BIRTH" width="100" />
          <el-table-column prop="mob" label="MOB" width="120" />
        </el-table>
      </div>
...
import { importPatient, type ImportResult } from '@/api/patient'
...
const previewData = ref<{ namePat: string; code: string; sdSex: string; nameSex: string; dtBirth: string; mob: string }[]>([])
...
const parsePreview = async (file: File) => {
  try {
    ...
    const sdSexIdx = headers.findIndex(h => h === 'sd_sex')
    const nameSexIdx = headers.findIndex(h => h === 'name_sex')
    const dtBirthIdx = headers.findIndex(h => h === 'dt_birth')
    const mobIdx = headers.findIndex(h => h === 'mob')
    ...
    for (let i = 1; i < Math.min(lines.length, 11); i++) {
      ...
      previewData.value.push({
        ...
        code: codeIdx >= 0 ? values[codeIdx] || '' : '',
        sdSex: sdSexIdx >= 0 ? values[sdSexIdx] || '' : '',
        nameSex: nameSexIdx >= 0 ? values[nameSexIdx] || '' : '',
        dtBirth: dtBirthIdx >= 0 ? values[dtBirthIdx] || '' : '',
        mob: mobIdx >= 0 ? values[mobIdx] || '' : ''
      })
    }
  } catch (error) {
...
const handleImport = async () => {
  ...
  try {
    const result = await importPatient(selectedFile.value)
