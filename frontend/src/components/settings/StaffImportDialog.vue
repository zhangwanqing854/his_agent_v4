    ...
    <div class="import-content">
      ...
      <div v-if="previewData.length > 0" class="preview-section">
        ...
        <el-table :data="previewData" border size="small" max-height="300">
          <el-table-column prop="codePsn" label="CODE_PSN" width="120" />
          <el-table-column prop="namePsn" label="NAME_PSN" />
          <el-table-column prop="codeUser" label="CODE_USER" width="120" />
          <el-table-column prop="nameUser" label="NAME_USER" />
          <el-table-column prop="sdEmptitle" label="SD_EMPTITLE" width="120" />
          <el-table-column prop="nameEmptitle" label="NAME_EMPTITLE" width="120" />
        </el-table>
      </div>
...
import { importHisStaff, type ImportResult } from '@/api/hisStaff'
...
const previewData = ref<{ codePsn: string; namePsn: string; codeUser: string; nameUser: string; sdEmptitle: string; nameEmptitle: string }[]>([])
...
const parsePreview = async (file: File) => {
  try {
    ...
    const codePsnIdx = headers.findIndex(h => h === 'code_psn')
    const namePsnIdx = headers.findIndex(h => h === 'name_psn')
    const codeUserIdx = headers.findIndex(h => h === 'code_user')
    const nameUserIdx = headers.findIndex(h => h === 'name_user')
    const sdEmptitleIdx = headers.findIndex(h => h === 'sd_emptitle')
    const nameEmptitleIdx = headers.findIndex(h => h === 'name_emptitle')
    ...
    for (let i = 1; i < Math.min(lines.length, 11); i++) {
      ...
      previewData.value.push({
        codePsn: codePsnIdx >= 0 ? values[codePsnIdx] || '' : '',
        namePsn: namePsnIdx >= 0 ? values[namePsnIdx] || '' : '',
        codeUser: codeUserIdx >= 0 ? values[codeUserIdx] || '' : '',
        nameUser: nameUserIdx >= 0 ? values[nameUserIdx] || '' : '',
        sdEmptitle: sdEmptitleIdx >= 0 ? values[sdEmptitleIdx] || '' : '',
        nameEmptitle: nameEmptitleIdx >= 0 ? values[nameEmptitleIdx] || '' : ''
      })
    }
  } catch (error) {
...
const handleImport = async () => {
  ...
  try {
    const result = await importHisStaff(selectedFile.value)
